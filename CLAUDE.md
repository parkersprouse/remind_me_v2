...
- **`src-tauri/gen/android/` contains hand edits.** `MainActivity.kt` intercepts back presses via `OnBackPressedCallback` and evaluates `window.androidBackHandler()` in the webview (bridge in `src/lib/androidBack.ts`). The manifest also has manual permission additions. Don't regenerate or blindly overwrite this tree.
